...
public class Sender implements Runnable {
    ...
    void run(long now) {
        ...
        // 假如有unknownLeadersExist,则调用Metadata的requestUpdate方法，标记需要更新Kafka的集群信息
        ...
        while (iter.hasNext()) {
            Node node = iter.next();
            // 循环调用NetworkClient.ready()方法，目的是检查网络I/O方面是否符合发送消息的条件。
        ...
        // 将KafkaChannel.send字段中保存的ClientRequest发送出去，
    ...
    private ClientRequest produceRequest(long now, int destination, short acks, int timeout, List<RecordBatch> batches) {
        // 注意produceRecordsByPartition和recordsByPartition的value是不一样的类。
        Map<TopicPartition, ByteBuffer> produceRecordsByPartition = new HashMap<TopicPartition, ByteBuffer>(batches.size());
        final Map<TopicPartition, RecordBatch> recordsByPartition = new HashMap<TopicPartition, RecordBatch>(batches.size());
        // 将RecordBatch列表按照partition分类，整理成上述两个集合
